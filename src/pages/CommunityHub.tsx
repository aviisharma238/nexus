...
const CommunityHub: React.FC = () => {
    ...
    try {
      ...
      if (response.data.user.role === "admin") {
        const createdResponse = await axios.get(
          `${API_BASE_URL}/communities/admin/my-communities`,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
        ...
        const allCommunitiesResponse = await axios.get(
  ...
  const postAnnouncement = async () => {
    try {
      const token = getToken();
      await axios.post(
        `${API_BASE_URL}/communities/${selectedCommunity?._id}/announcements`,
        newAnnouncement,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const postMessage = async (channel: string) => {
    try {
      const token = getToken();
      await axios.post(
        `${API_BASE_URL}/communities/${selectedCommunity?._id}/chat`,
  ...
  const leaveCommunity = async () => {
    ...
    try {
      const token = getToken();
      await axios.post(
        `${API_BASE_URL}/communities/${selectedCommunity._id}/leave`,
        {},
        {
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  const updateMemberRole = async (memberId: string, newRole: string) => {
    try {
      const token = getToken();
      await axios.put(
        `${API_BASE_URL}/communities/${selectedCommunity?._id}/members/${memberId}/role`,
        { role: newRole },
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const removeMember = async (memberId: string) => {
    ...
    try {
      const token = getToken();
      await axios.delete(
        `${API_BASE_URL}/communities/${selectedCommunity?._id}/members/${memberId}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
  ...
  const canAccessCommunity = (community: Community) => {
    return (
      communities.some((c) => c._id === community._id) ||
  ...
  const renderChannelContent = () => {
    ...
    `}
            ...
            <div className="card">
              <h3 style={{ fontSize: "1.25rem" }}>Community Guidelines</h3>
              <ul
                style={{
                  marginTop: "1rem",
                  paddingLeft: "1.5rem",
            ...
            <div className="card" style={{ marginTop: "1.5rem" }}>
              <h3 style={{ fontSize: "1.25rem" }}>Quick Stats</h3>
              <div
                style={{
                  display: "grid",
              ...
              >
                ...
                <div style={{ textAlign: "center" }}>
                  ...
                  >
                    {communityData.queries?.filter((q) => q.status === "open")
                      .length || 0}
                  </div>
            ...
            {userRole === "admin" && (
              <div className="card" style={{ marginTop: "1.5rem" }}>
                <h3 style={{ fontSize: "1.25rem" }}>Manage Community</h3>
                {isAdminOfCommunity(selectedCommunity!) ? (
                  <div style={{ marginTop: "1rem" }}>
                    <h4>
                      Members ({communityData.community.members?.length || 0})
                    </h4>
                    <div
                      style={{
                        maxHeight: "300px",
                    ...
                    >
                      {communityData?.community.members?.map((member) => (
                        <div
                        ...
                        >
                          ...
                          >
                            ...
                            <div>
                              ...
                              >
                                {communityData?.community?.adminId ===
                                member.userId?._id
                                  ? "Creator"
                                  : member.role || "Member"}
                              </div>
                  ...
                  >
                    ...
                    <p>
                      ...
                      {communityData?.community?.adminId?.name ||
                        "Another admin"}
                    </p>
                  </div>
                )}
      ...
      case "introductions":
        return (
          ...
          >
            ...
            >
              {communityData.chatMessages
                ?.filter((msg: any) => msg.messageType === "introduction")
                .map((msg: any) => {
                  const isMyMessage =
                    msg.userId?._id === localStorage.getItem("userId");
                  return (
                    <div
                      key={msg._id}
                      style={{
                      ...
                      >
                        ...
                        >
                          <strong style={{ color: "#4f46e5" }}>
                            {isMyMessage ? "You" : msg.userId?.name}
                          </strong>
                          <span
            ...
            <div style={{ display: "flex", gap: "0.5rem" }}>
              ...
              />
              <button
                onClick={() => postMessage("introductions")}
        ...
        return (
          <div style={{ padding: "1.5rem" }}>
            <div className="flex-between" style={{ marginBottom: "1.5rem" }}>
              ...
              >
                Role: {userRole} | UserId: {localStorage.getItem("userId")} |
                AdminId: {JSON.stringify(communityData?.community?.adminId)}
              </div>
              {isAdminOfCommunity(selectedCommunity!) && (
                <button
                  onClick={() =>
                    navigate(
                      `/community/${selectedCommunity?._id}/create-event`
                    )
                  }
                  className="btn-primary"
                >
            ...
            <div className="grid grid-auto">
              {communityData?.community?.events
                ?.filter(
                  (event: any) =>
                ...
                .map((event: any) => (
                  <div key={event._id} className="card">
                    ...
                    <div style={{ margin: "1rem 0" }}>
                      <p>
                        <strong>Theme:</strong> {event?.theme || "N/A"}
                      </p>
                      <p>
          ...
          >
            ...
            >
              {communityData?.queries?.map((query: any) => {
                const borderColor =
                  query?.status === "open" ? "#3b82f6" : "#10b981";
                return (
                  <div
                    key={query._id}
                    style={{
            ...
            <div>
              ...
              <div style={{ display: "flex", gap: "0.5rem" }}>
                <textarea
                  ...
                  style={{ flex: 1, minHeight: "60px", resize: "vertical" }}
                />
                <button onClick={submitQuery} className="btn-primary">
                  Submit
                </button>
              </div>
            </div>
      ...
      case "feedback":
        return (
          ...
          >
            ...
            >
              {communityData.chatMessages
                ?.filter((msg: any) => msg.messageType === "feedback")
                .map((msg: any) => {
                  const isMyMessage =
                    msg.userId?._id === localStorage.getItem("userId");
                  return (
                ...
                <p style={{ textAlign: "center", color: "#6b7280" }}>
                  Share your feedback about events and community.
                </p>
              )}
            </div>
            <div>
      ...
      case "updates":
        return (
          ...
          >
            ...
            >
              {communityData.chatMessages
                ?.filter((msg: any) => msg.messageType === "updates")
                .map((msg: any) => {
                  const isMyMessage =
                    msg.userId?._id === localStorage.getItem("userId");
                  return (
  ...
  return (
        ...
        >
            ...
            >
              ...
              <div
                style={{
                  ...
                  fontWeight: "bold",
                  color:
                    selectedCommunity?._id === community._id ? "#fff" : "#000",
                  boxShadow:
                    selectedCommunity?._id === community._id
          ...
          <button
            ...
            style={{
              ...
              fontSize: "0.9rem",
              fontWeight: "600",
               background: "linear-gradient(45deg, #222, #555)", // Darker theme
              border: "none",
              color: "white",
              cursor: "pointer",
              transition: "all 0.3s ease",
    ...
    `}
                  ...
                  {channels.map((channel) => {
                    ...
                    return (
                      <div
                        key={channel.id}
                        onClick={() =>
                          canAccess && setActiveChannel(channel.id)
                        }
                        style={{
      ...
      >
        {selectedCommunity ? (
          canAccessCommunity(selectedCommunity) ? (
          ...
          <div className="flex-center" style={{ height: "100%" }}>
            <div style={{ textAlign: "center" }}>
              <h2>Welcome to Nexus Communities</h2>
      ...
      {selectedCommunity && (
        ...
        >
          ...
          <div>
            {communityData?.community.members
              ?.slice(0, 10)
              .map((member: any) => (
                <div
                  key={member.userId?._id || member._id}
                ...
                >
                  ...
                  >
                    {(member.userId?.name || member.name || "U")
                      ?.charAt(0)
                      .toUpperCase()}
                  </div>
                  <div>
                    <div style={{ fontSize: "0.875rem", color: "white" }}>
                      {member.userId?.name || member.name || "Unknown User"}
                    </div>
                    <div
                      style={{
                        fontSize: "0.75rem",
